    ...
    : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        var response = await _paymentDispatcher.CreatePayment(
            request.Items
        ...
        CancellationToken ct)
    {
        ...
        await _paymentDispatcher.CancelSubscription(
            workTokenContext.UserId,
            ct);
        ...
        CancellationToken ct)
    {
        ...
        return Ok(_paymentMapper.ToResponse(await _paymentDispatcher.GetPayment(id, ct)));
    }
        ...
        CancellationToken ct)
    {
        ...
        var product = await _paymentDispatcher.GetProduct(
            productId,
            workTokenContext.UserId,
            ct);

        return Ok(_productMapper.ToResponse(product));
    }

    [Authorize]
    [HttpGet("premium/{premiumId}")]
    public async Task<IActionResult> GetPremiumPayment(
    [FromRoute] Guid premiumId,
    [FromServices] IWorkTokenContext workTokenContext,
    [FromServices] IClientContext clientContext,
    CancellationToken ct)
    {
        ...
        var payment = await _paymentDispatcher.GetPremiumPayment(
            premiumId,
            ct);

        return Ok(_paymentMapper.ToResponse(payment));
    }
}